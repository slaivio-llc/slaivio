...
def manifest(o,d,a='system',n='Slaivio'):
 ...
 with engine.begin() as c:ev(c,o,d,'MANIFEST_GENERATED',a,n,{'manifest_reference':manifest_reference,'packages':len(item['packages']),'weight_kg':sum(float(p.get('weight_kg') or 0) for p in item['packages']),'volume_cbm':sum(float(p.get('volume_cbm') or 0) for p in item['packages'])})
 return '\ufeff'+out.getvalue()
def analytics(o):
