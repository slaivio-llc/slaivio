...
export function DashboardOverviewPage() {
  ...
  return (
    <div className="flex h-dvh overflow-hidden bg-[#f8fafc] text-[#11182d]">
      ...
      <div className="min-w-0 flex-1 overflow-y-auto">
        <MobileHeader open={() => setSidebarOpen(true)} />
        <main className="mx-auto max-w-[1500px] px-4 py-6 sm:px-7 lg:px-9 lg:py-8">
          <PageHeader data={data} query={query} setQuery={setQuery} searchInput={searchInput} panel={panel} setPanel={setPanel} />
...
function Sidebar({ open, compact, unread, workspace, close, toggleCompact }: { open: boolean; compact: boolean; unread: number; workspace?: DashboardHome["workspace"]; close: () => void; toggleCompact: () => void }) {
  return <><button aria-label="Fermer le menu" onClick={close} className={`fixed inset-0 z-40 bg-slate-950/30 lg:hidden ${open ? "block" : "hidden"}`} /><aside className={`fixed inset-y-0 left-0 z-50 flex shrink-0 flex-col border-r border-slate-200 bg-white transition-all lg:relative lg:z-auto ${open ? "translate-x-0" : "-translate-x-full lg:translate-x-0"} ${compact ? "w-[76px]" : "w-[266px]"}`}>
    <div className="flex h-[88px] items-center px-5"><Image src="/slaivio-icon-official.png" width={42} height={42} alt="Slaivio" className="rounded-lg" />{!compact && <span className="ml-2 text-2xl font-bold tracking-tight">Slaivio</span>}<button onClick={toggleCompact} aria-label="Réduire le menu" className="ml-auto hidden rounded-lg border border-slate-200 p-2 text-slate-600 lg:block">{compact ? <ChevronRight size={17} /> : <ChevronLeft size={17} />}</button><button onClick={close} className="ml-auto rounded-lg p-2 lg:hidden"><X size={19} /></button></div>
    <nav className="min-h-0 flex-1 overflow-y-auto px-3 pb-4">{navGroups.map((group) => <div key={group.label} className="mb-5">{!compact && <p className="mb-2 px-3 text-[10px] font-semibold tracking-wide text-slate-500">{group.label}</p>}<div className="space-y-1">{group.items.map(([label, Icon, href]) => { const active = href === "/app"; return <a key={href} href={href} title={compact ? label : undefined} className={`flex min-h-10 items-center gap-3 rounded-lg px-3 text-sm ${active ? "bg-emerald-50 font-medium text-emerald-700" : "text-slate-700 hover:bg-slate-50"}`}><Icon size={18} /><span className={compact ? "hidden" : ""}>{label}</span>{label === "WhatsApp Inbox" && unread > 0 && !compact && <span className="ml-auto rounded-full bg-emerald-100 px-2 py-0.5 text-[11px] text-emerald-700">{unread}</span>}</a>; })}</div></div>)}</nav>
    <div className="m-3 rounded-xl border border-slate-200 p-3">{compact ? <Building2 className="mx-auto text-emerald-600" size={21} /> : <><p className="text-[11px] text-slate-500">Workspace actif</p><div className="mt-2 flex items-center gap-2"><span className="flex h-8 w-8 items-center justify-center rounded-lg bg-emerald-50 text-emerald-600"><Building2 size={17} /></span><span className="min-w-0 flex-1 truncate text-sm font-semibold">{workspace?.name || "Slaivio"}</span><ChevronDown size={15} /></div></>}</div>
  </aside></>;
}

function MobileHeader({ open }: { open: () => void }) { return <div className="sticky top-0 z-30 flex h-16 items-center border-b border-slate-200 bg-white px-4 lg:hidden"><button onClick={open} aria-label="Menu"><Menu size={22} /></button><Image src="/slaivio-icon-official.png" width={30} height={30} alt="" className="ml-4 rounded" /><span className="ml-2 font-bold">Slaivio</span></div>; }

function PageHeader({ data, query, setQuery, searchInput, panel, setPanel }: { data: DashboardHome | null; query: string; setQuery: (value: string) => void; searchInput: React.RefObject<HTMLInputElement | null>; panel: Panel; setPanel: (value: Panel) => void }) {
  return <header className="relative flex flex-wrap items-start justify-between gap-5"><div><h1 className="text-3xl font-bold tracking-[-0.035em]">Accueil</h1><p className="mt-2 text-sm text-slate-600">Retrouvez vos espaces et reprenez les opérations de {data?.workspace.name || "votre agence"}.</p></div><div className="flex w-full items-center gap-2 sm:w-auto"><label className="flex min-w-0 flex-1 items-center rounded-xl border border-slate-200 bg-white px-4 py-3 shadow-sm sm:w-[360px]"><Search size={17} className="text-slate-400" /><input ref={searchInput} value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Rechercher client, dossier, tracking…" className="ml-2 min-w-0 flex-1 bg-transparent text-sm outline-none" /><kbd className="hidden text-xs text-slate-400 md:block">Ctrl K</kbd></label><button onClick={() => setPanel(panel === "help" ? null : "help")} aria-label="Aide" className="rounded-xl border border-slate-200 bg-white p-3 text-slate-600"><CircleHelp size={18} /></button><button onClick={() => setPanel(panel === "notifications" ? null : "notifications")} aria-label="Notifications" className="relative rounded-xl border border-slate-200 bg-white p-3 text-slate-600"><Bell size={18} />{!!data?.unread_count && <span className="absolute right-2 top-2 h-2 w-2 rounded-full bg-red-500 ring-2 ring-white" />}</button><button onClick={() => setPanel(panel === "account" ? null : "account")} className="flex h-11 w-11 items-center justify-center rounded-full bg-[#10b953] text-sm font-bold text-white">{data?.manager.initials || "SL"}</button></div></header>;
}
