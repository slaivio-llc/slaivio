...
export function OperationMetrics({ children, className = "" }: { children: ReactNode; className?: string }) {
  return <section data-ui="operation-metrics" className="operation-metrics bg-white"><div className={`mx-auto w-full max-w-[1200px] border-b border-[#dfe3e7] px-6 py-3 sm:px-8 ${className}`}>{children}</div></section>;
}
...
export function OperationToolbar({
  ...
  return (
    <div data-ui="operation-toolbar" className="operation-toolbar bg-white">
      <div className={`mx-auto flex min-h-[54px] w-full max-w-[1200px] flex-col gap-2 border-b border-[#e1e5e9] px-6 py-2 sm:px-8 lg:flex-row lg:items-center ${className}`}>
...
export function ActiveFilterBar({ children }: { children?: ReactNode }) {
  ...
  return <div data-ui="active-filter-bar" className="operation-filter-bar bg-[#fafbfb]"><div className="mx-auto flex min-h-10 w-full max-w-[1200px] flex-wrap items-center gap-2 border-b border-[#e5e8eb] px-6 py-2 sm:px-8">{children}</div></div>;
}
